...
export default Mixin.create({
  ...
  getComponentWarning(component, componentCpu, componentMemory, displayComponent, ) {
    ...
    let prefix = 'clusterPage.insufficientSize.selectors'

    if (nodeSelectors.length === 0) {
      prefix = 'clusterPage.insufficientSize.workload'
    } else {
      ...
      if (unsupportedSelectors.length > 0) {
        return intl.t(`clusterPage.insufficientSize.selectors.unsupported`, { component: displayComponent ? displayComponent : parseCamelcase(component), })
      }
    }
